...
- (void)initSubviews {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_showBottomPreview) {
            [self.view addSubview:self.bottomPreView];
        }
    });
}
...
- (void)viewDidLayoutSubviews {
    ...
    if (_showBottomPreview) {
        frame = CGRectMake(0, top, _collectionView.frame.size.width, _collectionView.frame.size.height-100);
...
- (void)previewButtonClick {
    ...
    if (_showBottomPreview) {
        ...
        [photoPreviewVc setBackButtonClickBlock:^(BOOL isSelectOriginalPhoto) {
            weself.isSelectOriginalPhoto = isSelectOriginalPhoto;
            [weself.collectionView reloadData];
            [weself refreshBottomToolBarStatus];
            [weself refreshBottomPerview:tzImagePickerVc];
        }];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            //add
            if (_showBottomPreview) {
                [weakSelf refreshBottomPerview:tzImagePickerVc];
            }
...
//add
-(void)refreshBottomPerview:(TZImagePickerController *)tzImagePickerVc{
    for (UIView *view in self.bottomPreView.subviews) {
        [view removeFromSuperview];
    }
    __weak typeof (self)weself = self;
    for (int i= 0; i<tzImagePickerVc.selectedModels.count; i++) {
        PHAsset *asset = ((TZAssetModel *)tzImagePickerVc.selectedModels[i]).asset;
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            ...
            TZPhotoPickerPreviewButton *button = [TZPhotoPickerPreviewButton returnTZPhotoPickerPreviewButtonWithFrame:CGRectMake(10+i*80, 5, 80, 90) andImage:image andTagert:self];
            //            button.tag = i;
            button.model = tzImagePickerVc.selectedModels[i];
            button.selected = YES;
            button.modelImage = image;
            [self.bottomPreView addSubview:button];
            //点击底栏图片预览
            [button setBottomImageButtonShowPreView:^(UIButton *btn) {
                //                TZPhotoPreviewController *photoPreviewVc = [[TZPhotoPreviewController alloc] init];
                //                [self pushPhotoPrevireViewController:photoPreviewVc];
                //                //add
                //                if (_showBottomPreview) {
                ...
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否发送该图片" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
                UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    NSLog(@"****************发送图片");
//                    NSMutableArray *muArray = [NSMutableArray array];
//                    for (id obj in weself.bottomPreView.subviews) {
//                        if ([obj isKindOfClass:[TZPhotoPickerPreviewButton class]]) {
//                            TZPhotoPickerPreviewButton *buttonObj = obj;
//                            [muArray addObject:buttonObj.modelImage];
//                        }
//                    }
                    TZPhotoPickerPreviewButton *buttonObj = btn;
                    weself.pushImageToOthers(buttonObj.modelImage);
                }];
            ...
            //取消图片
            [button setCancelSelectPhotoBlock:^(TZAssetModel *model){
                for (TZAssetModel *models in tzImagePickerVc.selectedModels) {
                    if (models == model) {
                        models.isSelected = NO;
                        [tzImagePickerVc.selectedModels removeObject:models];
                        break;
                    }
                }
                if (_showBottomPreview) {
    ...
    self.bottomPreView.contentSize = CGSizeMake(tzImagePickerVc.selectedModels.count*100, 1);
}
...
-(UIView *)bottomPreView{
    if (!_bottomPreView) {
        _bottomPreView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-44-100-5, [UIScreen mainScreen].bounds.size.width, 100)];
        _bottomPreView.backgroundColor = [UIColor grayColor];
    }
